...
if (isset($data['message'])) {
    $query = $conn->query("SELECT * FROM `readings` WHERE `chatId` = {$chatId}");
    
    if ($query->num_rows == 0) {
        $query = $conn->query("INSERT INTO `readings` (`chatId`, `reading`) VALUES ({$chatId}, {$text})");
        } else {
            $query = $conn->query("UPDATE `readings` SET `reading` = {$text} WHERE `chatId` = {{$chatId}}");
    }
    } 
    */
